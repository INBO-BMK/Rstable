...
## Install lubridate
RUN  ./cran_install.sh lubridate 1.7.3
...
RUN  ./cran_install.sh yaml 2.1.18 \
...
RUN  ./cran_install.sh numDeriv 2016.8-1 \
  ...
  && ./cran_install.sh dfoptim 2017.12-1 \
...
RUN ./cran_install.sh xml2 1.2.0 \
  ...
  && ./cran_install.sh desc 1.1.1 \
  && ./cran_install.sh commonmark 1.4 \
...
## install mvtnorm
RUN  ./cran_install.sh mvtnorm 1.0-7
...
## install pander
RUN  ./cran_install.sh pander 0.6.1

## install DT
RUN  ./cran_install.sh htmlwidgets 1.0 \
  && ./cran_install.sh httpuv 1.3.6.2 \
...
## install osmar
RUN  ./cran_install.sh XML 3.98-1.10 \
  && ./cran_install.sh RCurl 1.95-4.10 \
...
RUN apt-get update \
  && apt-get install  -y --no-install-recommends \
    ...
    libproj-dev \
  && apt-get clean \
  && ./cran_install.sh rgdal 1.2-18

## install bookdown and webshot
RUN  ./cran_install.sh xfun 0.1 \
  && ./cran_install.sh bookdown 0.7 \
...
## install dbplyr
RUN  ./cran_install.sh dbplyr 1.2.1
